 ...
 * 
 */
@OpenAPIDefinition(info = @Info(title = "Cambyze financial service", version = "0.0", description = "Services to perform financial calculation", license = @License(name = "Apache 2.0", url = "https://cambyze.com/"), contact = @Contact(url = "https://cambyze.com/", name = "cambyze", email = "https://cambyze.com/")))
@RestController
public class EffectiveRateFromCashFlowController {

	private static final Logger LOGGER = LoggerFactory.getLogger(EffectiveRateFromCashFlowController.class);
	...
	public BigDecimal calculateRate(@RequestBody EffectiveRateFromCashFlow effectiveRateFromCashFlow) {
		...
		for (Payment p : paymentsCollection) {
			...
			cashFlow.put(LocalDate.parse(p.getDate(), europeanDateFormatter), BigDecimal.valueOf(p.getAmount() * -1.0));
		}
		...
		for (Payment p : paymentsCollection) {
			...
			cashFlow.put(LocalDate.parse(p.getDate(), europeanDateFormatter), BigDecimal.valueOf(p.getAmount()));
		}

		LOGGER.info("cashFlow :" + cashFlow + " / isActualDay: " + effectiveRateFromCashFlow.getIsActualDays());
